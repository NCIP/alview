...
#if 1
// rpf
				fprintf(stderr, "bcfview.c Option '-T' can only take value trioauto, trioxd or trioxs.\n");
#else
	...
	if ((vc.flag & VC_VCFIN) && (vc.flag & VC_BCFOUT) && vc.fn_dict == 0) {
		// rpf fprintf(stderr, "[%s] For VCF->BCF conversion please specify the sequence dictionary with -D\n", __func__);
		fprintf(stderr, "[] For VCF->BCF conversion please specify the sequence dictionary with -D\n");
		return 1;
	}
	...
	if (vc.flag & VC_CALL) {
		...
		if (vc.prior_file) {
			if (bcf_p1_read_prior(p1, vc.prior_file) < 0) {
				// rpf fprintf(stderr, "[%s] fail to read the prior AFS.\n", __func__);
				fprintf(stderr, "[ fail to read the prior AFS.\n");
				return 1;
			}
	...
	if (optind + 1 < argc && !(vc.flag&VC_VCFIN)) {
		...
		if (bcf_parse_region(str2id, argv[optind+1], &tid, &begin, &end) >= 0) {
			...
			if (idx) {
				...
				if (off == 0) {
					// rpf fprintf(stderr, "[%s] no records in the query region.\n", __func__);
					fprintf(stderr, "no records in the query region.\n");
	...
	while (vcf_read(bp, hin, b) > 0) {
		...
		if (vc.flag & VC_CALL) { // call variants
			...
			if (n_processed % 100000 == 0) {
				// rpf fprintf(stderr, "[%s] %ld sites processed.\n", __func__, (long)n_processed);
				fprintf(stderr, "%ld sites processed.\n",  (long)n_processed);
				bcf_p1_dump_afs(p1);
			}
