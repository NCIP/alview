...
void bcf_idx_save(const bcf_idx_t *idx, BGZF *fp)
{
	...
	if (ti_is_be) {
		uint32_t x = idx->n;
		// rpf bgzf_write(fp, bam_swap_endian_4p(&x), 4);
		bgzf_write(fp, (const void *)bam_swap_endian_4p(&x), 4);
...
{
#if 1
fprintf(stderr,"rpf commented out bcf_idx_load_local() \n"); fflush(stderr);
return 0;
#else
	BGZF *fp;
	// rpf fp = bgzf_open(fnidx, "r");
	fp = bgzf_open(fnidx, (const char *)"r");
	if (fp) {
