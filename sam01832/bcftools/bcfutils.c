...
{
#if 1
fprintf(stderr,"rpf bcf_str2id commented out for const problem \n"); fflush(stderr);
return 0;
#else
	khash_t(str2id) *hash = (khash_t(str2id)*)_hash;
	khint_t k;
...
{
#if 1
fprintf(stderr,"rpf bcf_str2id_add commented out for const problem \n"); fflush(stderr);
return 0;
#else
	khint_t k;
	int ret;
...
bcf_hdr_t *bcf_hdr_subsam(const bcf_hdr_t *h0, int n, char *const* samples, int *list)
{
	...
	if (j < n) fprintf(stderr, "%d samples in the list but not in BCF.",  n - j);
	kh_destroy(str2id, hash);
...
int bcf_gl10(const bcf1_t *b, uint8_t *gl)
{
	...
	for (k = 0; k < 4; ++k)
		if (map[k] < 0) map[k] = k1;
	for (i = 0; i < b->n_smpl; ++i) {
		// rpf const uint8_t *p = PL->data + i * PL->len; // the PL for the i-th individual
