...
// fprintf(stderr,"RPF before bam_fetch zzz is_count=%d idx=%d tid=%d beg=%d end=%d\n",is_count,idx,tid,beg,end);  fflush(stderr); 
				result = bam_fetch(in->x.bam, idx, tid, beg, end, out, view_func);
// fprintf(stderr,"RPF after bam_fetch aaa result = %d %p\n",result,result);  fflush(stderr); 
                        }
			if (result < 0) {
