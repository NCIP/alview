...
// fprintf(stderr,"RPF in bam_iter_query idx=%d tid=%d beg=%d end=%d\n",idx,tid,beg,end);  fflush(stderr); 
	if (beg < 0) beg = 0;
	if (end < beg) return 0;
	// initialize iter
...
// fprintf(stderr,"RPF in bam_iter_query 2\n");  fflush(stderr); 
	if (n_off == 0) {
		free(bins); return iter;
	}
// fprintf(stderr,"RPF in bam_iter_query 3\n");  fflush(stderr); 
	...
	free(bins);
// fprintf(stderr,"RPF in bam_iter_query 4\n");  fflush(stderr); 
	if (n_off == 0) {
		free(off); return iter;
	}
// fprintf(stderr,"RPF in bam_iter_query mid\n");  fflush(stderr); 
	{
...
// fprintf(stderr,"RPF end bam_iter_query %p %d\n",iter,(int)iter);  fflush(stderr); 
	return iter;
}
...
{
	int ret;
// fprintf(stderr,"RPF in bam_iter_read\n");  fflush(stderr); 
...
// fprintf(stderr,"RPF in bam_iter_read 2, ret=%d\n",ret);  fflush(stderr); 
	for (;;) {
// fprintf(stderr,"RPF in bam_iter_read 3\n");  fflush(stderr); 
		if (iter->curr_off == 0 || iter->curr_off >= iter->off[iter->i].v) { // then jump to the next chunk
			if (iter->i == iter->n_off - 1) { ret = -1; break; } // no more chunks
			if (iter->i >= 0) assert(iter->curr_off == iter->off[iter->i].v); // otherwise bug
			if (iter->i < 0 || iter->off[iter->i].v != iter->off[iter->i+1].u) { // not adjacent chunks; then seek
// fprintf(stderr,"RPF in bam_iter_read 3.2\, seek to %ld \n",iter->off[iter->i+1].u);  fflush(stderr); 
				
				bam_seek(fp, iter->off[iter->i+1].u, SEEK_SET);
// fprintf(stderr,"RPF in bam_iter_read 3.3\, seeked to %ld \n",iter->curr_off);  fflush(stderr); 
		...
		if ((ret = bam_read1(fp, b)) >= 0) {
			iter->curr_off = bam_tell(fp);
// fprintf(stderr,"RPF in bam_iter_read 3.4\, now at %ld \n",iter->curr_off);  fflush(stderr); 
...
// fprintf(stderr,"RPF in bam_iter_read 4\n");  fflush(stderr); 
	iter->finished = 1;
// fprintf(stderr,"RPF in bam_iter_read 5, ret=%d \n",ret);  fflush(stderr); 
	return ret;
}
...
int bam_fetch(bamFile fp, const bam_index_t *idx, int tid, int beg, int end, void *data, bam_fetch_f func)
{
	...
	b = bam_init1();
	iter = bam_iter_query(idx, tid, beg, end);
// fprintf(stderr,"RPF after bam_iter_query\n");  fflush(stderr); 
	while ((ret = bam_iter_read(fp, iter, b)) >= 0) func(b, data);
// fprintf(stderr,"RPF after bam_iter_read , ret=%d\n",ret);  fflush(stderr); 
